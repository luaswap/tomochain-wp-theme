...
<div class="<?php echo esc_attr( trim( $css_class ) ); ?>">
    <div class="dapp-<?php echo esc_attr($dapp_layout);?>" <?php if('slide' == $dapp_layout):?>data-atts="<?php echo esc_attr( json_encode( $atts ) ); ?>"<?php endif;?>>
        <?php tomochain_category_filter( 'dapp' );?>
        <?php if( $dapps->have_posts() ):
                ...
                <div class="tomochain-dapp-item">
                    ...
                    <div class="dapp-info">
                        <h4 class="dapp-title text-truncate">
                            <?php echo the_title(); ?>
                        </h4>
                        <div class="dapp-content">
                            <?php the_content();?>
                            <?php if($custom_url):?>
                                <a class="more-info" href="<?php echo esc_url($custom_url)?>" target="<?php echo esc_attr($open_new_tab); ?>"><?php echo esc_html__('More Info','nootheme')?></a>
                            <?php endif;?>
                            <?php if($contract_address_url):?>
                                <a href="<?php echo esc_url($contract_address_url)?>" target="<?php echo esc_attr($open_new_tab); ?>"><?php echo esc_html__('Contract Address','nootheme')?></a>
                            <?php endif;?>
                        </div>
                    </div>
                </div>
